...
class Game:
    ...
    def play(self) -> None:
        ...
        while True:
            ...
            if command == 'work' and self.working == False:
                print(red + "You don't  have any job.  How will you work?")
    ...
    def shop(self):
        ...
        {blue}\nHere are the commands you can use:
        To see the list of commands, type [help]
        {bright_blue}{red}︻デ═一{bright_blue}Hunting Rifle, {yellow}ID: [rifle], {magenta}Command to use: [hunt], {cyan}Price: ⏣ 15,000
            \t\t{white}Use rifle to hunt and sell what you find.
        \t{bright_blue}🎣  Fishing Rod, {yellow}ID: [rod], {magenta}Command to use: [fish], {cyan}Price: ⏣ 15,000
            \t\t{white}Use rod to hunt and sell what you find.
        \t{bright_blue}   Multiplier, {yellow}ID: [multiplier], {magenta}Command to use: [multiply], {cyan}Price: ⏣ 35,000
    ...
    def inventory_display(self):
        str1 = ","
        all_elements = str1.join(self.inventory)
        ...
        if len(self.inventory) > 0:
            ...
            print(f"{bright_blue}Here is your inventory: ")
    ...
    def sell(self):
        ...
        while selling_choice in self.inventory:
            ...
            if selling_choice == 'rifle' and 'rifle' in self.inventory:
                if self.inventory.count('rifle') >= 1:
                    print("You have sold your Rifle for ⏣ 9000")
                    self.money += 9000

                if self.inventory.count('rifle') > 1:
                    try:
                        how_many_sell = int(input("How many rifles would you like to sell?\n>"))
                        for number in range(how_many_sell):
                            self.inventory.remove('rifle')
                            break
